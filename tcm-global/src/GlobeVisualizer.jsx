...
const REGION_REGISTRY = {
  TW: {
    ...
    outerSvgPath:
      "M 180 20 L 200 40 L 210 70 L 205 100 L 190 130 L 170 150 L 150 165 L 130 170 L 110 160 L 95 140 L 90 120 L 95 95 L 110 70 L 130 50 L 155 35 Z",
    ...
    middleRegions: [
      {
        id: "north",
        name: "北部",
        path: "M 256 50 L 350 80 L 380 150 L 360 200 L 300 220 L 250 200 L 220 150 L 240 100 Z",
        sites: [
      ...
      },
      {
        id: "central",
        name: "中部",
        path: "M 200 200 L 280 220 L 300 300 L 280 380 L 220 400 L 180 380 L 160 300 L 180 250 Z",
        sites: [
      ...
      },
      {
        id: "south",
        name: "南部",
        path: "M 180 380 L 260 400 L 280 480 L 240 500 L 200 480 L 160 450 L 140 400 L 160 380 Z",
        sites: [
...
function sphericalQuadGeometry({
  lat0,
  lat1,
  lon0,
  lon1,
  r = LAYER_RADIUS.OUTER,
  seg = 6,
}) {
  const positions = [];
  const normals = [];
...
function TileOnSphere({
  ...
  const geo = useMemo(
    ...
    [lat0, lat1, lon0, lon1, layerRadius]
  );

  const center = useMemo(
    () => ll2xyz((lat0 + lat1) / 2, (lon0 + lon1) / 2, layerRadius + 0.022),
    [lat0, lat1, lon0, lon1, layerRadius]
  );
              ...
              {region.middleRegions.map((reg) => (
                <g key={reg.id}>
                  <path
                    ...
                    onClick={() =>
                      onRegionPick &&
                      onRegionPick({ region: reg, parentData: data })
                    }
                    onMouseEnter={(e) => (e.target.style.fill = "#bae6fd")}
                    onMouseLeave={(e) =>
                      (e.target.style.fill = "url(#middleFill)")
                    }
                  />
                  <text
                    x={reg.path.split(" ")[1]}
                  ...
                  {reg.sites.map((site) => (
                    ...
                    >
                      <circle
                        ...
                        strokeWidth="1.5"
                      />
                      <text x="6" y="3" fill="#0f172a" fontSize="9">
                        {site.label}
                      </text>
                    </g>
                  ))}
        ...
        <Html transform position={[0, 0, 0.005]} distanceFactor={2.8}>
          ...
          >
            ...
            >
              ...
              {shortZh(parentData.zh) || parentData.en || parentData.code}
            </div>

            {/* 內層詳細地圖 */}
            <svg
              viewBox={regionData.innerViewBox}
              style={{ width: "100%", height: "100%" }}
            >
              <defs>
              ...
              {regionData.innerSites.map((site) => (
                <g key={site.id} transform={`translate(${site.x}, ${site.y})`}>
                  ...
                  {/* 容量標示 */}
                  <text x="10" y="16" fill="#059669" fontSize="8">
                    {site.cap}MW
                  </text>
                </g>
              ))}
...
const MultiLayerCameraRig = ({ phase, targetLayer, targetDir, onArrive }) => {
  ...
  useFrame((_, dt) => {
    ...
    const layerPositions = {
      ...
      middle: targetDir
        ? targetDir.clone().multiplyScalar(LAYER_RADIUS.MIDDLE + 1.5)
...
const MultiLayerScene = ({ options }) => {
  ...
  const targetDir =
    outerSelection?.center?.clone().normalize() || new THREE.Vector3(0, 0, 1);
  ...
  return (
    <>
      ...
      <OrbitControls
        ...
        enableZoom
        minDistance={
          currentLayer === "inner" ? 1.5 : currentLayer === "middle" ? 2.0 : 2.5
        }
        maxDistance={
          currentLayer === "inner"
            ? 4.0
